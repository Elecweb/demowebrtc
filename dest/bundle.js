...
const setupVideo = (element, stream) => {
...
var index$2 = function parseuri(str) {
    var src = str,
        b = str.indexOf('['),
        e = str.indexOf(']');
...
var removeBlobs = function(data, callback) {
  function _removeBlobs(obj, curKey, containingObject) {
    ...
    } else if (index$14(obj)) { // handle array
...
Encoder.prototype.encode = function(obj, callback){
  if ((obj.type === exports.EVENT || obj.type === exports.ACK) && index$10(obj.data)) {
...
var xmlhttprequest = function (opts) {
  ...
  try {
    if ('undefined' !== typeof XMLHttpRequest && (!xdomain || index$22)) {
...
var index$24 = function(arraybuffer, start, end) {
...
exports.encodePacket = function (packet, supportsBinary, utf8encode, callback) {
  ...
  } else if (index$28 && data instanceof commonjsGlobal.Blob) {
...
function encodeBlob(packet, supportsBinary, callback) {
  ...
  var blob = new index$28([length.buffer, packet.data]);
...
exports.encodeBase64Packet = function(packet, callback) {
  ...
  if (index$28 && packet.data instanceof commonjsGlobal.Blob) {
    var fr = new FileReader();
    fr.onload = function() {
...
exports.decodePacket = function (data, binaryType, utf8decode) {
  ...
  var type = asArray[0];
  var rest = index$24(data, 1);
  if (index$28 && binaryType === 'blob') {
    rest = new index$28([rest]);
  }
...
exports.decodeBase64Packet = function(msg, binaryType) {
  ...
  if (binaryType === 'blob' && index$28) {
    data = new index$28([data]);
  }
...
exports.encodePayload = function (packets, supportsBinary, callback) {
  ...
  var isBinary = index$10(packets);

  if (supportsBinary && isBinary) {
    if (index$28 && !dontSendBlobs) {
...
function map(ary, each, done) {
  ...
  var next = index$26(ary.length, done);
...
exports.encodePayloadAsBlob = function(packets, callback) {
  function encodeOne(packet, doneCallback) {
    exports.encodePacket(packet, true, true, function(encoded) {
      ...
      if (index$28) {
        var blob = new index$28([binaryIdentifier.buffer, lengthAry.buffer, encoded]);
  ...
  map(packets, encodeOne, function(err, results) {
    return callback(new index$28(results));
  });
};
...
exports.decodePayloadAsBinary = function (data, binaryType, callback) {
  ...
  while (bufferTail.byteLength > 0) {
    ...
    bufferTail = index$24(bufferTail, 2 + msgLength.length);
    msgLength = parseInt(msgLength);

    var msg = index$24(bufferTail, 0, msgLength);
    if (isString) {
      try {
    ...
    buffers.push(msg);
    bufferTail = index$24(bufferTail, msgLength);
  }
...
Polling.prototype.uri = function () {
  ...
  if (false !== this.timestampRequests) {
    query[this.timestampParam] = index$34();
  }
...
index$32(JSONPPolling, polling$1);
...
var index$38 = function parsejson(data) {
...
function Socket (uri, opts) {
  ...
  if (uri) {
    ...
    opts.hostname = index$2(opts.host).host;
  }
  ...
  this.port = opts.port || (commonjsGlobal.location && location.port
      ...
      : (this.secure ? 443 : 80));
  this.query = opts.query || {};
  if ('string' === typeof this.query) this.query = index$30.decode(this.query);
...
Socket.prototype.createTransport = function (name) {
  ...
  var transport$$1 = new index$20[name]({
    query: query,
    socket: this,
...
Socket.prototype.onPacket = function (packet) {
  if ('opening' === this.readyState || 'open' === this.readyState ||
    ...
    switch (packet.type) {
      case 'open':
        this.onHandshake(index$38(packet.data));
        break;
...
Socket.prototype.filterUpgrades = function (upgrades) {
  ...
  for (var i = 0, j = upgrades.length; i < j; i++) {
    if (~index$36(this.transports, upgrades[i])) filteredUpgrades.push(upgrades[i]);
  }
  return filteredUpgrades;
};

var index$18 = socket$1;
...
index$18.parser = parser;

var index$16 = index$18;

var index$41 = toArray;
...
var emit = index$8.prototype.emit;
...
Socket.prototype.subEvents = function () {
  ...
  var io = this.io;
  this.subs = [
    on_1(io, 'open', index$43(this, 'onopen')),
    on_1(io, 'packet', index$43(this, 'onpacket')),
    on_1(io, 'close', index$43(this, 'onclose'))
  ];
};
...
Socket.prototype.emit = function (ev) {
  ...
  var args = index$41(arguments);
  var packet = { type: index$6.EVENT, data: args };
...
Socket.prototype.onopen = function () {
  ...
  if ('/' !== this.nsp) {
    if (this.query) {
      var query = typeof this.query === 'object' ? index$30.encode(this.query) : this.query;
      debug('sending connect packet with query %s', query);
      this.packet({type: index$6.CONNECT, query: query});
    } else {
      this.packet({type: index$6.CONNECT});
    }
  }
};
...
Socket.prototype.onpacket = function (packet) {
  ...
  switch (packet.type) {
    ...
    case index$6.BINARY_ACK:
      this.onack(packet);
      break;

    case index$6.DISCONNECT:
      this.ondisconnect();
      break;
...
Socket.prototype.disconnect = function () {
  if (this.connected) {
    ...
    this.packet({ type: index$6.DISCONNECT });
  }
...
function Manager (uri, opts) {
  ...
  this.encoding = false;
  this.packetBuffer = [];
  var _parser = opts.parser || index$6;
...
Manager.prototype.connect = function (fn, opts) {
  debug$3('readyState %s', this.readyState);
  if (~this.readyState.indexOf('open')) return this;

  debug$3('opening %s', this.uri);
  this.engine = index$16(this.uri, this.opts);
  ...
  if (false !== this._timeout) {
    ...
    debug$3('connect attempt will timeout after %d', timeout);

    // set timer
    var timer = setTimeout(function () {
      debug$3('connect attempt timed out after %d', timeout);
      openSub.destroy();
      socket.close();
...
Manager.prototype.onopen = function () {
  ...
  // add new subs
  var socket = this.engine;
  this.subs.push(on_1(socket, 'data', index$43(this, 'ondata')));
  this.subs.push(on_1(socket, 'ping', index$43(this, 'onping')));
  this.subs.push(on_1(socket, 'pong', index$43(this, 'onpong')));
  this.subs.push(on_1(socket, 'error', index$43(this, 'onerror')));
  this.subs.push(on_1(socket, 'close', index$43(this, 'onclose')));
  this.subs.push(on_1(this.decoder, 'decoded', index$43(this, 'ondecoded')));
};
...
Manager.prototype.socket = function (nsp, opts) {
  ...
  if (!socket) {
    socket = new socket$3(this, nsp, opts);
  ...
  function onConnecting () {
    if (!~index$36(self.connecting, socket)) {
...
Manager.prototype.destroy = function (socket) {
  var index = index$36(this.connecting, socket);
...
Manager.prototype.packet = function (packet) {
  debug$3('writing packet %j', packet);
  var self = this;
...
Manager.prototype.reconnect = function () {
  ...
  if (this.backoff.attempts >= this._reconnectionAttempts) {
    debug$3('reconnect failed');
    this.backoff.reset();
    this.emitAll('reconnect_failed');
    this.reconnecting = false;
  } else {
    var delay = this.backoff.duration();
    debug$3('will wait %dms before reconnect attempt', delay);

    this.reconnecting = true;
    var timer = setTimeout(function () {
      if (self.skipReconnect) return;

      debug$3('attempting reconnect');
      ...
      self.open(function (err) {
        if (err) {
          debug$3('reconnect attempt error');
          self.reconnecting = false;
          self.reconnect();
          self.emitAll('reconnect_error', err.data);
        } else {
          debug$3('reconnect success');
          self.onreconnect();
        }
      });
    }, delay);
...
exports.protocol = index$6.protocol;
...
const GET_ICE = 'GET_ICE';
const SEND_ICE = 'SEND_ICE';
